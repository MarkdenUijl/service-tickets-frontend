...
const measureNow = () => {
  ...
  visibleCount.value = Math.max(
    1,
    Math.floor((viewportWidth + gap) / (cards[0].offsetWidth + gap))
  )
  ...
  currentIndex.value = Math.min(
    currentIndex.value,
    Math.max(0, props.cards.length - 1)
  )
}

const cardStyle = computed(() =>
  cardDynamicWidth.value ? { flex: `0 0 ${cardDynamicWidth.value}px` } : {}
)
...
onMounted(() => {
  ...
  if (trackEl.value)
    mutationObserver.observe(trackEl.value, {
      childList: true,
      subtree: true,
      attributes: true
    })
...
const setIndex = (newIndex) => {
  currentIndex.value = Math.min(Math.max(0, newIndex), props.cards.length - 1)
}

const carouselForward = () => setIndex(currentIndex.value + 1)
const carouselBackward = () => setIndex(currentIndex.value - 1)
</script>
    ...
    >
      ...
      >
        <div
          ...
          :style="cardStyle"
        >
          <span class="carousel-card-title">{{ card.cardTitle }}</span>
          <span class="carousel-card-info">{{ card.cardInfo }}</span>
        </div>
      </div>
    </div>
...
/* When both are true, fade both edges */
.carousel.can-go-back.can-go-next {
...
.carousel.can-go-back:not(.can-go-next) {
  mask-image: linear-gradient(
    to right,
    rgba(0, 0, 0, 0) 0%,
    rgba(0, 0, 0, 1) 2.5%
...
.carousel.can-go-next:not(.can-go-back) {
  mask-image: linear-gradient(
    to right,
    rgba(0, 0, 0, 1) 97.5%,
    rgba(0, 0, 0, 0) 100%
